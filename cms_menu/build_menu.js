...
Hybrid.version="1.2.1"; // this version will center web apps and uses local Storage and have a singular object.!
...
function _hybridInit(){
   ...
   // here we do a top-domain and referer check!
	//Hybrid.debugmessage("encoded "+Hybrid.encodeString("ERROR: Referer mismatch! This game has not been licensed to run on this site! Contact www.snoep.at to obtain the proper licenses."));
   if(!_hybridLeechProtect())
   {
	// someone is trying to leech, or I just forgot to set anything....
		Hybrid.showWarning(Hybrid.decodeString(""));
...
Hybrid.showWarning=hybrid_warning;
function hybrid_warning(message)
{
...
function _hybridListenToTilt(tf,uth)
{
	...
	{
		Hybrid.showWarning("Your device doesn't seem to be able to detect tilt.");
	}else
	{
...
function _hybridMakeSingleDraggable(o,down,up,drag,move)
{
	...
	if (window.navigator.msPointerEnabled) 
	{
	  // Pointer events are supported.
		htsp.addEventListener("MSPointerMove", _genericDragHandler, false);
		htsp.addEventListener("MSPointerDown", _genericDragHandler, false);
		htsp.addEventListener("MSPointerUp", _genericDragHandler, false);
		if(typeof(move)!=="undefined")
		{
			Hybrid.debugmessage("Draggable AND mousemove enabled (windows tablet)!");
			htsp.addEventListener("MSPointerOver", _genericDragHandler, false);
//			htsp.addEventListener("mousemove", _genericDragHandler, false); // this might to be necessary to get windos 8 to play along
		}
	...
	window.onmouseup=_genericDragHandler; // this is reported to even catches mouse up outside window!
	htsp.onmousedown=_genericDragHandler;
	htsp.onmouseup=_genericDragHandler;
	htsp.onmousemove=_genericDragHandler;
	htsp.ontouchstart=_genericDragHandler;
	htsp.ontouchmove=_genericDragHandler;
	htsp.ontouchend=_genericDragHandler;
	htsp.ontouchcancel=_genericDragHandler;	
...
var _genericDrag={};
_genericDrag.lastXY={};
function _genericDragHandler(eventObject)
{
	//Hybrid.debugmessage("_genericDragHandler eventObject.type "+eventObject.type);
...
//	position: relative;
	...
	tb.jquery.on('keyup change', function() {_hybridTextboxChanged(tb,"change");} ); // we should let the hybrid know which textbox is being edited!
	tb.jquery.on('focus', function() {_hybridTextboxChanged(tb,"focus");} ); // we should let the hybrid know which textbox is being edited!
	tb.jquery.on('blur', function() {_hybridTextboxChanged(tb,"blur");} ); // we should let the hybrid know which textbox is being edited!
...
function _hybridCrWeVw(l,x,y,w,h,url,co) // configObject NOT supported yet!
{
  ...
  o.jquery=$("#"+o.id).css("z-index",l.dynamic_element_counter);
  o.ifr=$("#"+o.id+"_if");
...
*/
	...
	Hybrid.aatd.push("687474703a2f2f7370656c2e6b696a6b6f70737465656e62657267656e2e6e6c2f6d67636d732f"); //http://spel.kijkopsteenbergen.nl/mgcms/
//	Hybrid.debugmessage("aatd: "+Hybrid.encodeString("http://spel.kijkopsteenbergen.nl/mgcms/"));
	...
	var tool_width=200;
	//var tool_height=32;
	Hybrid.graphics_manifest['tools']={src:"tools.png",w:600,h:288,preload: true,kind:"sprite",ss:{}}; // we preload it, because it's in the splash!
		Hybrid.graphics_manifest['tools'].ss['vragen'      ]=[ [0,  0,tool_width,32,0,0,0],[tool_width,  0,tool_width,32,0,0,0] ,[tool_width*2,  0,tool_width,32,0,0,0] ];// reg x and y =0 for buttons!
		Hybrid.graphics_manifest['tools'].ss['media'       ]=[ [0, 32,tool_width,32,0,0,0],[tool_width, 32,tool_width,32,0,0,0] ,[tool_width*2, 32,tool_width,32,0,0,0] ];
		Hybrid.graphics_manifest['tools'].ss['kaart'       ]=[ [0, 64,tool_width,32,0,0,0],[tool_width, 64,tool_width,32,0,0,0] ,[tool_width*2, 64,tool_width,32,0,0,0] ];
		Hybrid.graphics_manifest['tools'].ss['huizen'      ]=[ [0, 96,tool_width,32,0,0,0],[tool_width, 96,tool_width,32,0,0,0] ,[tool_width*2, 96,tool_width,32,0,0,0] ];
		Hybrid.graphics_manifest['tools'].ss['extra1'      ]=[ [0,128,tool_width,32,0,0,0],[tool_width,128,tool_width,32,0,0,0] ,[tool_width*2,128,tool_width,32,0,0,0] ];
		Hybrid.graphics_manifest['tools'].ss['extra2'      ]=[ [0,160,tool_width,32,0,0,0],[tool_width,160,tool_width,32,0,0,0] ,[tool_width*2,160,tool_width,32,0,0,0] ];
		Hybrid.graphics_manifest['tools'].ss['statistieken']=[ [0,192,tool_width,32,0,0,0],[tool_width,192,tool_width,32,0,0,0] ,[tool_width*2,192,tool_width,32,0,0,0] ];
		Hybrid.graphics_manifest['tools'].ss['bugs'        ]=[ [0,224,tool_width,32,0,0,0],[tool_width,224,tool_width,32,0,0,0] ,[tool_width*2,224,tool_width,32,0,0,0] ];
		Hybrid.graphics_manifest['tools'].ss['disclaimer'  ]=[ [0,256,tool_width,32,0,0,0],[tool_width,256,tool_width,32,0,0,0] ,[tool_width*2,256,tool_width,32,0,0,0] ];
	...
	a=[[1,1,82,71,0,37.3,25.4],[84,1,82,67,0,37.3,21.4],[167,1,82,66,0,37.3,21.4],[250,1,82,67,0,37.3,22.4],[333,1,81,71,0,36.3,26.4],[415,1,82,72,0,36.3,27.4],[498,1,68,73,0,36.3,28.4],[567,1,69,74,0,36.3,29.4],[637,1,68,75,0,36.3,30.4],[706,1,67,75,0,36.3,30.4],[774,1,67,74,0,36.3,29.4],[842,1,66,69,0,36.3,25.4],[909,1,66,65,0,36.3,20.4],[1,77,67,63,0,37.3,19.4],[69,77,68,63,0,37.3,19.4],[138,77,70,63,0,37.3,19.4],[209,77,72,64,0,38.3,20.4],[282,77,75,62,0,38.3,18.4],[358,77,72,63,0,38.3,19.4],[431,77,71,63,0,39.3,19.4],[503,77,69,63,0,39.3,19.4],[573,77,74,63,0,39.3,19.4],[648,77,80,63,0,40.3,19.4],[729,77,81,63
6],[284,627,60,68,0,15.299999999999997,-40.6],[345,627,55,72,0,13.299999999999997,-38.6],[401,627,48,75,0,9.299999999999997,-37.6],[450,627,40,75,0,5.299999999999997,-37.6],[491,627,33,75,0,1.2999999999999972,-37.6],[525,627,25,76,0,-2.700000000000003,-37.6],[551,627,18,76,0,-6.700000000000003,-37.6],[570,627,19,75,0,-6.700000000000003,-38.6],[590,627,25,74,0,-3.700000000000003,-38.6],[616,627,35,72,0,0.29999999999999716,-39.6],[652,627,45,70,0,5.299999999999997,-40.6],[698,627,53,66,0,9.299999999999997,-43.6],[752,627,61,60,0,13.299999999999997,-46.6],[814,627,68,54,0,17.299999999999997,-49.6],[883,627,72,47,0,19.299999999999997,-53.6],[1,704,76,39,0,21.299999999999997,-57.6],[589,550,77,34,0,22.299999999999997,-59.6],[78,704,76,32,0,22.299999999999997,-60.6],[155,704,72,36,0,20.299999999999997,-58.6],[228,704,67,43,0,17.299999999999997,-54.6],[296,704,0,0,0,496.3,233.4],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[297,704,53,53,0,8.299999999999997,-65.6],[351,704,53,53,0,8.299999999999997,-65.6],[405,704,53,52,0,8.299999999999997,-66.6],[459,704,53,52,0,8.299999999999997,-66.6],[513,704,51,50,0,7.299999999999997,-67.6],[565,704,51,46,0,7.299999999999997,-69.6],[617,704,50,45,0,6.299999999999997,-70.6],[668,704,48,42,0,5.299999999999997,-72.6],[717,704,48,41,0,5.299999999999997,-73.6],[766,704,48,39,0,5.299999999999997,-74.6],[815,704,48,39,0,5.299999999999997,-74.6],[864,704,48,39,0,5.299999999999997,-74.6],[913,704,48,39,0,5.299999999999997,-74.6],[962,704,48,39,0,5.299999999999997,-74.6],[1,758,48,40,0,5.299999999999997,-73.6],[50,758,48,41,0,5.299999999999997,-73.6],[99,758,48,42,0,5.299999999999997,-72.6],[148,758,49,42,0,6.299999999999997,-72.6],[198,758,50,44,0,6.299999999999997,-71.6],[249,758,50,46,0,6.299999999999997,-70.6],[300,758,49,47,0,6.299999999999997,-69.6],[350,758,51,49,0,7.299999999999997,-68.6],[402,758,51,50,0,7.299999999999997,-67.6],[454,758,52,50,0,8.299999999999997,-67.6],[507,758,53,52,0,8.299999999999997,-66.6],[297,704,53,53,0,8.299999999999997,-65.6],[561,758,102,102,0,50.3,50.4]];	
...
//	Hybrid.disablePageScrolling(); // as a default, we want this to act as a webapp on iPad and other tablets!
	...
	function showLoadingProgress(perc,first_time)
	{
		...
		if(first_time)
		{
			...
			Hybrid.createTextBox(bg,0,Hybrid.height/2-30,Hybrid.width,75,'sans-serif',colorPalet.loading_bar,"center",50,"Laden..");
			...
			Hybrid.createTextBox(bg,0,Hybrid.height-130,Hybrid.width,75,'sans-serif',"#999999","center",20,"<a href='"+Hybrid.helplink+"'>(Meld een probleem)</a>");
